...
class Image {
	...
	*/
	 ...
	 * @return string
	 */
	public function upload($file, $dir = null, $createDimensions = true)
	{
	    if ($file)
	    {
	    	$dir_name = date('Y') . "/" . date('m') . "/" . date('d');
	        // Generate random dir
	        if ( ! $dir)
	        	$dir = $dir_name;
	 		else
	        ...
	        $filename 		= date('d').".".date('m').".".date('Y')."_".$filename_string.'.'.File::extension($destination.'/'.$filename);
